...
const TARGET=+process.argv[2]||60, BUDGET=(+process.argv[3]||440)*1000, OUT=process.argv[4]||'p.json';
const gen=`
...
function selfPlay(maxPlies){let b=initialBoard(),pl=1;const pos=[];
  for(let ply=0;ply<maxPlies;ply++){const mv=allMoves(b,pl);if(!mv.length)break;
    ...
    if(!forced)pos.push({board:clone(b),player:pl});
    let pick;
    if(forced){pick=mv[Math.floor(Math.random()*mv.length)];}
    else if(Math.random()<0.32){pick=rootEval(b,pl,2)[0].mv;}
    else{pick=mv[Math.floor(Math.random()*mv.length)];}
...
const DEEP=5,PRE=3,SWING=110,MAXACC=3;
const seen=new Set(),puz=[],t0=Date.now();let games=0;
while(puz.length<TARGET&&(Date.now()-t0)<BUDGET){games++;const pos=selfPlay(48);
  for(const P of pos){ if((Date.now()-t0)>BUDGET)break;
    ...
    const pre=rootEval(P.board,P.player,PRE);if(pre[0].val-pre[1].val<50)continue;
    const sc=rootEval(P.board,P.player,DEEP);const best=sc[0].val,second=sc[1].val,swing=best-second;
  ...
  if(games%25===0)console.error(OUT+' games='+games+' puz='+puz.length+' t='+Math.round((Date.now()-t0)/1000));}
console.error(OUT+' DONE games='+games+' puz='+puz.length);
RES=JSON.stringify(puz);`;
ctx.TARGET=TARGET;ctx.BUDGET=BUDGET;ctx.OUT=OUT;vm.runInContext(gen,ctx);
fs.writeFileSync(OUT,ctx.RES);
